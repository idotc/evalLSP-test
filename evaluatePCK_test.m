function evaluatePCK_test(predidxs,evalMode,bSave)
...
% load ground truth
assert(strcmp(evalMode,'PC1') ||strcmp(evalMode,'PC') || strcmp(evalMode,'OC'));
...
for i = 1:length(predidxs);
    ...
    % plot results
    [row, header] = genTablePCK(pck(end,:),'LSP');
    tableTex{1} = header;
    tableTex{i+1} = row;
